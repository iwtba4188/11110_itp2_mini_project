...
// check has assign or not
int check_assign(AST* root);
// combine constant
...
int check_assign(AST* root) {
    ...
    if (root->kind == ASSIGN || root->kind == PREINC || root->kind == PREDEC || root->kind == POSTINC || root->kind == POSTDEC) return 1;
    else {
        return check_assign(root->lhs) || check_assign(root->mid) || check_assign(root->rhs);
    }
}

// int reg[256] = {0};    // 0 unused, 1 used
int reg = 0;
int i_reg[3] = {-1};
int draw_reg() {
    return (reg++ % 256);
...
int codegen(AST* root, Kind parent_kind) {    // -> return @register_number if exists else return -1
    ...
    //     if (parent_kind != ASSIGN && root->kind == IDENTIFIER) {    // need to load
    ...
    int this_r = draw_reg();
    AST* deepest_id = root->lhs;
    switch (root->kind) {
        case CONSTANT:
            if (root->val >= 0) printf("add r%d 0 %d\n", this_r, root->val);
            else printf("sub r%d 0 %d\n", this_r, root->val);
            break;
        case IDENTIFIER:
            printf("load r%d [%d]\n", this_r, (root->val - 'x') * 4);
            break;
        case ASSIGN:
            if (root->lhs != NULL && root->lhs->kind == LPAR) {
                while (deepest_id->kind != IDENTIFIER)
                    deepest_id = deepest_id->mid;
            }
            printf("store [%d] r%d\n", (deepest_id->val - 'x') * 4, r_r);
            printf("load r%d [%d]\n", this_r, (deepest_id->val - 'x') * 4);
            break;
        case ADD:
        case SUB:
        case MUL:
        case DIV:
        case REM:
            printf("%s r%d r%d r%d\n", KindName[root->kind], this_r, r_l, r_r);
            break;
        case PREINC:
            deepest_id = root->mid;
            if (root->mid != NULL && root->mid->kind == LPAR) {
                while (deepest_id->kind != IDENTIFIER)
                    deepest_id = deepest_id->mid;
            }
            printf("add r%d r%d 1\n", this_r, r_m);
            printf("store [%d] r%d\n", (deepest_id->val - 'x') * 4, this_r);
            break;
        case PREDEC:
            deepest_id = root->mid;
            if (root->mid != NULL && root->mid->kind == LPAR) {
